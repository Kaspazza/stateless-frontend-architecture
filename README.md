...
* [`src/stateless_frontend_architecture/`](src/stateless_frontend_architecture/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/stateless_frontend_architecture/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `stateless-frontend-architecture` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/stateless_frontend_architecture/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns stateless-frontend-architecture.example
  (:require [stateless-frontend-architecture.config :as config])
